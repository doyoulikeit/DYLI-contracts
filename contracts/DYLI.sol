...
function batchRedeem(
        ...
        bytes32 hash = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encodePacked(msg.sender, tokenIds, nonce, fees))
        );
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(fees[i] > 0) {
                require(usdc.transferFrom(msg.sender, feeRecipient, fees[i]), "Payment for shipping failed");
            }
